...
export default function HomeSteps() {
    return (
        ...
        >
            <h2 className="text-white text-3xl 2xl:text-5xl font-semibold leading-14 tracking-wide">¿Cómo funciona?</h2>
            <div className="flex flex-col lg:flex-row gap-4 lg:gap-16 w-full items-center justify-center">
                {stepsData.map((step, index) => (
                    <div 
                        key={index}
                        className="flex flex-col items-center justify-center h-auto min-h-56 lg:h-76
                                    shadow-lg shadow-[#6A4BE0]/20 hover:shadow-xl hover:shadow-[#41C7B5]/30 transition-shadow duration-300
                                    border border-[#71D4C4]/30 bg-linear-to-br from-[#6A4BE0]/10 via-[#2d1b4e]/20 to-[#41C7B5]/10
                                    backdrop-blur-sm rounded-3xl w-full p-4 lg:p-8 max-w-64"
                    >
                        ...
                        <div className="flex flex-col h-3/4 text-white text-center lg:text-start justify-between">
                            <h3 className="text-2xl font-semibold lg:h-1/2 mb-2">{step.title}</h3>
